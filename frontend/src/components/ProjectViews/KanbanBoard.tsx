...
const SortableTask: React.FC<{ task: Task; onTaskClick: (task: Task) => void }> = ({ task, onTaskClick }) => {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    if (over.id !== active.id) {
      ...
      if (overTask) {
        ...
        // Dropped on column container - extract status from container id if needed
        // This would need column droppable areas to work properly
      }
    }
              ...
              <div className="animate-pulse">
                ...
                <div className="space-y-3">
                  {Array.from({ length: 3 }).map((_, j) => (
                    <div key={j} className="h-32 bg-gray-200 dark:bg-gray-700 rounded-lg"></div>
                  ))}
                </div>
              </div>
            </div>
