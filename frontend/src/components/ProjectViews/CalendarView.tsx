...
const TaskEventCard: React.FC<{ task: Task; onClick?: () => void }> = ({ task, onClick }) => {
...
const CalendarView: React.FC<CalendarViewProps> = ({ workspaceId, onTaskClick }) => {
  ...
  const { tasksByDate, monthStats } = useMemo(() => {
    if (!tasks) return { tasksByDate: {}, monthStats: { total: 0, completed: 0, overdue: 0 } };
    ...
    tasks.forEach(task => {
      if (task.dueDate) {
        ...
        // Check if task is in current month view
        if (dueDate >= monthStart && dueDate <= monthEnd) {
          total++;
          if (task.status === 'DONE') completed++;
          if (dueDate < new Date() && task.status !== 'DONE') overdue++;
        }
          ...
          {dayTasks.slice(0, 3).map((task, index) => (
            <div
              ...
              className={`w-2 h-2 rounded-full ${
                ...
                overdueTasks > 0 ? 'bg-red-500' :
                task.status === 'IN_PROGRESS' ? 'bg-blue-500' : 'bg-gray-400'
              }`}
            />
          ))}
          {dayTasks.length > 3 && (
            <span className="text-xs text-gray-600">+{dayTasks.length - 3}</span>
          )}
        </div>
      </div>
    );
  ...
  const tileClassName = ({ date, view }: { date: Date; view: string }) => {
    ...
    if (dayTasks.length > 0) {
      ...
      const allCompleted = dayTasks.every(task => task.status === 'DONE');
