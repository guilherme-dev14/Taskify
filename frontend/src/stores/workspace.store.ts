...
interface WorkspaceState {
  ...
  updateMember: (workspaceId: number, memberId: number, updates: Partial<WorkspaceMember>) => void;
    ...
    (set, get) => ({
      ...
      removeMember: (workspaceId, memberId) =>
        set((state) => ({
          workspaces: state.workspaces.map((ws) =>
            ws.id === workspaceId
              ? { ...ws, members: ws.members.filter((member) => member.id !== memberId) }
              : ws
          ),
        })),
      ...
      createInvite: async (workspaceId) => {
        ...
        const invite: WorkspaceInvite = {
          code: inviteCode,
          workspaceName: get().workspaces.find(ws => ws.id === workspaceId)?.name || '',
      ...
      joinWorkspaceByInvite: async (code) => {
        ...
        // This would typically make an API call to join the workspace
        console.log('Joining workspace with code:', code);
      },

      revokeInvite: (workspaceId, code) =>
        set((state) => ({
          pendingInvites: state.pendingInvites.filter(inv => inv.code !== code),
        })),
      ...
      // Real-time handlers
      handleWorkspaceUpdate: (workspace) => get().updateWorkspace(workspace.id, workspace),

      handleMemberJoined: (workspaceId, member) => get().addMember(workspaceId, member),

      handleMemberLeft: (workspaceId, memberId) => get().removeMember(workspaceId, memberId),
